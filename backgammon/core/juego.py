"""Módulo que coordina el estado del juego, los dados y el turno actual."""
...
class Juego:
    ...
    def __init__(self, jugador1, jugador2, indice_inicial=0):
        ...
        idx = 0 if indice_inicial not in (0, 1) else indice_inicial
        self.__indice_jugador_actual__ = idx
    ...
    def _dado_mayor_que(self, distancia_minima: int) -> int | None:
        ...
        movs = [m for m in self.__movs_restantes__ if m >= distancia_minima]
        movs_validos = sorted(movs)
    ...
    def _validar_bearing_off(self, desde: int) -> tuple[bool, int]:
        """Extrae la lógica de validación de Bearing Off (hasta == PUNTOS)."""
        pid = self.jugador_actual.id

        if not self.__tablero__.puede_sacar_fichas(pid):
            msg = "solo podés sacar fichas si todas están en tu zona de salida"
            self._set_error(msg)
            return False, 0
        ...
        if self.es_ficha_mas_lejana(pid, desde):
            ...
            if dado_a_usar is None:
                msg = (
                    f"la distancia {distancia} no está en movs "
                    f"{self.__movs_restantes__} ni se puede sobrepasar"
                )
                self._set_error(msg)
                return False, distancia
            return True, dado_a_usar

        msg = "no se puede sobrepasar: hay fichas más lejos que requieren un dado menor"
        self._set_error(msg)
        return False, distancia
    ...
    def _validar_movimiento(self, desde: int, hasta: int) -> tuple[bool, int]:
        ...
        if desde < 0 or desde >= PUNTOS or hasta < 0 or hasta > PUNTOS:
            msg = f"índices fuera de rango (0..{PUNTOS-1} o {PUNTOS} para sacar)"
            self._set_error(msg)
            return False, 0
        ...
        if self._en_barra(pid):
            ...
            return self._validar_bearing_off(desde)
    ...
    def aplicar_movimiento(self, desde: int, hasta: int) -> bool:
        """Aplica el movimiento (mover, reingresar o sacar) y consume el dado."""
    ...
    def cambiar_turno(self):
        """Cambia el jugador actual, limpia los movimientos restantes y actualiza estado."""
    ...
    def estado_dict(self) -> dict:
        ...
        ids = [j.id for j in self.__jugadores__]
        barra = {pid: self.__tablero__.fichas_en_barra(pid) for pid in ids}
        salidas = {pid: self.__tablero__.fichas_salidas(pid) for pid in ids}
        return {
    ...
    def resumen_estado(self) -> str:
        ...
        e = self.estado_dict()
        return (
            f"estado={e['estado']} | turno={e['jugador_actual']} "
